    ...
    <el-row>
      ...
      >
        ...
        <el-table-column
          ...
          width="50px"/>
        <el-table-column
          :label="$t('workers.operateType')"
          prop="operateType"
          align="center"
          width="100px"/>
        <el-table-column
          :label="$t('workers.operateDesc')"
          prop="operateDesc"
          align="center"
          width="200px"/>
        <el-table-column
          :label="$t('workers.operateQty')"
          prop="operateQty"
          align="center"
          width="100px"/>
        <el-table-column
          :label="$t('workers.operateValue')"
          prop="operateValue"
          align="center"
          width="110px"/>
        <el-table-column
          ...
          width="100px"/>
        <el-table-column
          :label="$t('workers.returnData')"
          prop="returnData"
          align="center"
          width="100px"/>
        <el-table-column
          :label="$t('workers.error')"
          align="center"
          prop="error"/>
        <el-table-column
          :label="$t('workers.createTime')"
          prop="createTime"
          align="center"
          width="180px"/>
        <el-table-column
          :label="$t('workers.updateTime')"
          prop="updateTime"
          align="center"
          width="180px"/>
      </el-table>
...
import { fetchPage, setStatus } from '@/api/worker'
...
export default {
  ...
  methods: {
    initData: async function() {
      this.statusObj = await fetchEnumObj('WorkerStatus')
      this.optionData = await fetchEnumList('WorkerStatus')
      this.getMessage()
    },
    //  获取图表列表
    getMessage: function() {
    ...
    handleChoose: function() {
      ...
      for (var i = 0; i < this.currentRows.length; i++) {
        if (this.currentRows[i].status !== -10) {
          this.$message({
            message: '所选worker记录中含有状态不为失败的记录',
            type: 'error'
          })
          return false
        }
      ...
      this.loading = true
      setStatus({ 'workerIds': ids }).then((res) => {
        if (res.data.result) {
          this.getMessage()
        } else {
          this.loading = false
